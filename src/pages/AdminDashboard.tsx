      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-7 pb-40 space-y-8 no-scrollbar">
          <div className="bg-[#F8F9FA] rounded-[2rem] p-6 space-y-5 border border-gray-100">
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-11 h-11 bg-white rounded-2xl flex items-center justify-center text-gray-200 border border-gray-50 shadow-sm"><User size={20}/></div>
                  <div>
               ...
               {order.items?.map((item, i) => (
                 <div key={i} className="flex items-center gap-4 px-6 py-4">
                   ...
                   <div className="flex-1">
                     ...
                     {item.variant && <p className="text-[9px] text-gray-400 uppercase font-black mt-1 tracking-tighter">{item.variant.flavor} / {item.variant.weight}</p>}
                   </div>
          ...
          <div className="flex justify-center pt-4 pb-12">
             <button onClick={() => onDelete(order.id)} className="flex items-center gap-2 text-[10px] font-black text-gray-200 hover:text-red-500 transition-colors uppercase tracking-widest"><Trash2 size={14}/> Remove Order Record</button>
          </div>
        </div>
        <div className="absolute bottom-0 inset-x-0 p-6 bg-white border-t border-gray-50 pb-10">
           <button onClick={handleSave} disabled={saving} className="w-full bg-gray-950 text-white py-5 rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-2xl active:scale-95 transition-all">
...
export const AdminDashboard = () => {
  ...
  const handleBulkUpdate = async () => {
    ...
    toast.success('Batch Workflow Updated');
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[60vh] flex-col gap-4">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-300">Syncing Dashboard</p>
    </div>
  );
      ...
      <AnimatePresence>
        {selectedIds.length > 0 && (
          <motion.div
            ...
            className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[92%] max-w-[400px] bg-gray-950 rounded-[2.5rem] p-5 flex items-center gap-3 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] z-50 overflow-hidden"
          >
